...
if($cover_image && $pdf_file) {
    ...
    $empty_blob1 = '';
    $empty_blob2 = '';
    $stmt->bind_param("ssssssiddbsssssssi", $title, $author, $isbn, $genre, $description, $book_type, $total_quantity, $rental_price_per_day, $purchase_price, $empty_blob1, $cover_image_type, $empty_blob2, $pdf_file_name, $pdf_file_size, $pdf_file_type, $publisher, $publication_date, $admin_id);
    $stmt->send_long_data(9, $cover_image);
    $stmt->send_long_data(11, $pdf_file);
} elseif($cover_image) {
    ...
    $stmt = $conn->prepare($sql);
    $empty_blob = '';
    $stmt->bind_param("ssssssiddbsssi", $title, $author, $isbn, $genre, $description, $book_type, $total_quantity, $rental_price_per_day, $purchase_price, $empty_blob, $cover_image_type, $publisher, $publication_date, $admin_id);
    $stmt->send_long_data(9, $cover_image);
} elseif($pdf_file) {
    ...
    $stmt = $conn->prepare($sql);
    $empty_blob = '';
    $stmt->bind_param("ssssssiddbssssssi", $title, $author, $isbn, $genre, $description, $book_type, $total_quantity, $rental_price_per_day, $purchase_price, $empty_blob, $pdf_file_name, $pdf_file_size, $pdf_file_type, $publisher, $publication_date, $admin_id);
    $stmt->send_long_data(9, $pdf_file);
} else {
