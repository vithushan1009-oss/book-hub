...
if (!isset($_GET['token'])) {
    ...
    header('Location: ../../login.html?error=' . urlencode('Invalid verification link'));
    exit();
}
...
if ($result->num_rows == 0) {
    ...
    header('Location: ../../login.html?error=' . urlencode('Invalid or expired verification link'));
    exit();
}
...
if ($update_stmt->execute()) {
    ...
    header('Location: ../../login.html?success=' . urlencode('Email verified successfully! You can now log in.'));
} else {
    $_SESSION['error'] = 'Verification failed. Please try again.';
    header('Location: ../../login.html?error=' . urlencode('Verification failed. Please try again.'));
}
